...
void _insert(BTreeNode* present, int k)
{
	...
	else // If this node is not leaf
	{
		...
		//Write your code.
		int j=0;
		while(j<present->n&&present->keys[j]<k)
			j++;
...
void _remove(BTreeNode* present, int k)
{
	//-------------------------------------------------------------------------------------------------------
	//Write your code.
	present=_search(present, k);
	if(present==NULL){
		printf("didn't find %d in the tree", k);
		return;
	}
	else if(present->leaf==false){
		int removeIndex;
		for(removeIndex=0; removeIndex<present->n; removeIndex++){
			if(present->keys[removeIndex]==k)
				break;
		}
		BTreeNode* exchangeNode = _getMaxLeaf(present->C[removeIndex]);
		present->keys[removeIndex]=exchangeNode->keys[(exchangeNode->n)-1];
		(exchangeNode->n)--;
		_balancingAfterDel(exchangeNode);
		return;

	}
	else if(present->leaf==true){
		int i=0;
		while(present->keys[i]!=k)
			i++;
		for(;i<(present->n)-1; i++){
			present->keys[i]=present->keys[i+1];
		}
...
BTreeNode* _getMaxLeaf(BTreeNode* present){
	if(present->leaf==true){
		return present;
	}
	else{
		return _getMaxLeaf(present->C[present->n]);
	}
}